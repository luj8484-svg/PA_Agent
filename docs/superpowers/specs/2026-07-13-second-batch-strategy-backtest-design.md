...
- 单笔初始风险上限 0.5%，组合冻结开放风险上限 1%，组合回撤达到或超过 10% 后进入 `HALT_TRIGGERED → HALTED` 终态流程。
...
flowchart LR
    ...
    C --> P["2B ExecutionPlan / ExecutionRejection"]
    P --> E["2C 1m 事件引擎"]
    D --> E
...
4. 2B 在版本化目标执行分钟读取当时可见的 1m open；先形成独立计划草案，再做 gap、contract、成本、仓位、资金和组合风险判断，输出 ExecutionPlan 或 ExecutionRejection。
...
8. 任一路径 `INVALID`、`HALTED` 或未满足硬门槛，只能报告失败，不能被人工豁免为通过。

## 8. 确定性边界

- 价格、数量、费用、保证金、风险预算和账本统一使用 `Decimal`。
- EMA/ATR 内部允许严格锁定依赖版本的 `float64`；Donchian 使用原始 Decimal OHLC。
- float64 只在 Candidate 边界转换为 15 位有效数字、round-half-even 的 Decimal。
...
- `StrategyCandidate`：只回答市场是否出现 LONG/SHORT setup；不回答是否允许交易。
- `ValidationFailure`：数据、pre-roll、指标或版本不足；不能伪装为无 setup。
- `ExecutionPlan`：基于目标分钟可见数据，冻结 entry/stop/TP/quantity、成本和规则版本。
...
只允许指标数值函数、Golden Fixture、Candidate/ValidationFailure Schema 和确定性市场规则。禁止 Plan、仓位、成本、事件引擎、账本和绩效。
...
2. Candidate 的 `NO_TRADE`/`NO_SETUP` 兼容语义。
3. 生命周期 Schema 和 Canonical ID 输入。
4. 1m 事件优先级及所有同分钟组合。
